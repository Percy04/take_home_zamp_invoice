  ...
  return (
    <main>
      <section aria-labelledby="page-title">
        ...
        <p className="summary">
          Upload a synthetic invoice to compare it with purchase records and see
          every control behind the result.
        </p>
        ...
        <p className="privacy" role="note">
          ...
          extracted evidence goes to OpenAI; provider credentials remain on the
          server.
        </p>
      </section>
    </main>
...
function RunPage() {
  ...
  return (
    <main className="run-page">
      ...
      <section
        className={`decision ${detail.state === "POSTED" ? "success" : "processing"}`}
      >
        <p className="eyebrow">Decision</p>
        <h2>{detail.decision ?? "Processing evidence…"}</h2>
        <p>
          {detail.ledgerId
            ? `Posted once as ${detail.ledgerId}`
            : detail.reasonCode
              ? `${detail.reasonCode}: ${detail.nextAction}`
              : "The server is working through the persisted stages."}
        </p>
      </section>
      ...
      {detail.invoice && (
        <section>
          ...
          <dl className="facts">
            <div>
              <dt>Vendor</dt>
              <dd>{detail.invoice.vendor}</dd>
            </div>
            <div>
              <dt>PO</dt>
              <dd>{detail.invoice.poNumber}</dd>
            </div>
            <div>
              <dt>Subtotal</dt>
              <dd>${detail.invoice.subtotal}</dd>
            </div>
            <div>
              <dt>Tax</dt>
              <dd>${detail.invoice.tax}</dd>
            </div>
            <div>
              <dt>Total</dt>
              <dd>${detail.invoice.total}</dd>
            </div>
          </dl>
        </section>
      )}
