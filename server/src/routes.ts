...
const upload = multer({ storage: multer.memoryStorage(), limits: { files: 1, fileSize: maxPdfBytes } });

export function createApi(storage?: Storage) {
  const api = Router();

  api.get("/health", (_request, response) => {
    response.json({ status: "ok", database: storage ? "available" : "not-initialized" });
  });

  if (!storage) return api;

  api.post("/runs", upload.single("invoice"), async (request, response, next) => {
    try {
      ...
      if (fixtureId && fixtureId !== "happy") {
        return response.status(400).json(error("INVALID_FIXTURE", "Only the happy fixture is available."));
      }
