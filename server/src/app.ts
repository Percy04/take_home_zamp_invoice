...
export function createApp(
  ...
  if (env.NODE_ENV === "development")
    app.use(cors({ origin: env.DEV_CORS_ORIGIN, methods: ["GET", "POST"] }));
  ...
  app.use(
    (
      ...
      if (
        ...
        return response
          .status(400)
          .json(error("INVALID_REQUEST", "The request is invalid."));
      ...
      if (env.NODE_ENV === "development")
        console.error("[request]", response.locals.correlationId, safeServerError(err));
...
function validRunIdFromRequest(request: express.Request) {
  const value =
    request.params.runId ?? request.originalUrl.match(/\/runs\/([0-9a-f-]{36})/i)?.[1];
